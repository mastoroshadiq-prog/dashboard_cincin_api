"""
Dashboard Z-Score + Cincin Api + Yield v6.0
============================================
...
output_dir = Path(f'data/output/dashboard_v6_{timestamp}')
...
def generate_html(output_dir, all_results, all_maps, prod_df):
    ...
    for idx, (divisi, data) in enumerate(all_results.items()):
        ...
        for i, (_, r) in enumerate(top_gano.iterrows(), 1):
            yield_val = prod_df[prod_df['Blok_Prod'].str.contains(r['Blok'][:3], na=False)]['Yield_TonHa'].mean()
        ...
        if not prod_df.empty:
            ...
            for i, (_, r) in enumerate(low_yield.iterrows(), 1):
                blok_match = block_stats[block_stats['Blok'].str.contains(str(r['Blok_Prod'])[:3], na=False)]
    ...
    html = f'''<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Dashboard Z-Score + Cincin Api v6.0</title>
<style>
...
<body>
<div class="header"><h1>🔥 DASHBOARD Z-SCORE + CINCIN API v6.0</h1><p>Hybrid Detection dengan Korelasi Produktivitas</p></div>
<div class="container">
...
function switchTab(id) {{
    ...
    path = output_dir / 'dashboard_v6.html'
...
def main():
    print('='*70)
    print('🔥 DASHBOARD Z-SCORE + CINCIN API + YIELD v6.0')
    print('='*70)
